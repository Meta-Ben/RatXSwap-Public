...
const run = async () => {
  ...
  asciiLogo = await getAsciifyImage('./img/RatXSwap.jpg')

  let ascii = await getFigletData("$ RatXSwap $")
...
if(optionSelected == 1) {
  ...
  console.log("See you soon fat rat !")
  process.exit()
}
else {
...
let checkForLiquidityAndGetPairContract = async(optionSelected) => {
 ...
 /* const tokenInAbis = await axios.get('https://api.bscscan.com/api?module=contract&action=getabi&address='+data.TOKEN_IN);

  tokenInContract = new ethers.Contract(
    data.TOKEN_IN,
    tokenInAbis.data.result,
    account
  );*/
    ...
    //Here to force a pair adress ( to avoid pancakeswap anti bot returning 0x000 )
    //pairContractAdress = "0x1d34286da0f1435c12334610293cf1f8f4318211"
  ...
  let buyAction = async(optionSelected) => {
    ...
    try {
      ...
      console.log(
       ...
       if(optionSelected == 1) {
        console.log("Processing Swap BNB => Token..... ( Rat goes to the blockchain )'")
        ...
        console.log('Processing Swap Token => Token..... ( Rat goes to the blockchain )');
